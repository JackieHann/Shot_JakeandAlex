...
bool findSHOTonGoalSpeedAndAngle(float* speed, float* angle, float x)
{
	...
	float nextAngle(minAngle * toRads);	// Start with shallowest angle...
	...
	const float increment = (deltaD * toRads);

	float tanAngle = (0.314159265358979f);

	do
	{
		//float AngleRads = (nextAngle * (Pi / 180.0F));			// Need radians for cos and tan functions
		float nextSpeed = minSpeed;									// reset minimum speed 

		//TESTING

		float firstPart = 2.0F *cos(nextAngle) *cos(nextAngle);
		float xTanAngle = tanAngle * x;
		do
		{

			float height = (xSquaredGrav / (firstPart* (nextSpeed*nextSpeed))) + xTanAngle;	//Phew!
			...
			else
			{
				*speed = nextSpeed;			// Record the working combination...
				...
				int temp = (int)(nextAngle * toDegs * 2);
				*angle = (float)temp * 0.5f;
		...
		nextAngle += increment;	// no joy, try next angle up (+0.5 degrees).
